...
public:
  Connections(int theBallCount) :
      ...
      connectionsOfIndex(new QVector<QVector<int> *>[ballCount])
  {
      ...
      connectionsOfIndex(new QVector<QVector<int> *>[ballCount])
  {
    for (int i = 0;i < another.connections.size();++i)
      connections.push_back(new QVector<int>(*another.connections.at(i)));

    for (int i = 0;i < ballCount;++i)
    {
      connectionsOfIndex[i].reserve(another.connectionsOfIndex[i].size());
      for (int j = 0;j < another.connectionsOfIndex[i].size();++j)
      {
        if (another.connectionsOfIndex[i].at(j))
          connectionsOfIndex[i].push_back(
              new QVector<int>(*another.connectionsOfIndex[i].at(j)));
        else
